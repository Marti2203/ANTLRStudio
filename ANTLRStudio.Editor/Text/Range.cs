...
namespace ANTLRStudio.Editor.Text
{
    ...
    public class Range : IEnumerable<Place>
    {
        ...
        public int TextLength => ColumnSelectionMode ? Length_ColumnSelectionMode(true) : Length;
        ...
        internal void GoPageUp(bool shift)
        {
            ...
            if (preferedPos < 0)
                preferedPos = start.iChar - tb.LineInfos[start.iLine].GetWordWrapStringStartPosition(tb.LineInfos[start.iLine].GetWordWrapStringIndex(start.iChar));
            int pageHeight = tb.ClientSize.Height / tb.CharHeight - 1;
        ...
        => GetRangesByLines(new Regex(regexPattern, options));
