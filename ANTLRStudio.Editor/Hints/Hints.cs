...
namespace ANTLRStudio.Editor.Hints
{
    ...
    public class Hints : ICollection<Hint>, IDisposable
    {
        ...
        {
            if (e.Key == Keys.Escape && e.Modifiers == Keys.None)
                Clear();
        }
        ...
        private void LayoutHint(Hint hint)
        {
            if (hint.Inline)
            {
                if (hint.Range.Start.iLine < tb.LineInfos.Count - 1)
                    ...
                    hint.HostPanel.Top = tb.TextHeight + tb.Padding.Top - hint.HostPanel.Height - tb.VerticalScroll.Value;
            }
            else
            {
        ...
        /// </summary>
        public bool Contains(Hint item) => items.Contains(item);

        public void CopyTo(Hint[] array, int arrayIndex) => items.CopyTo(array, arrayIndex);
