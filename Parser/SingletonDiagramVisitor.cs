...
namespace ANTLRStudio.Parser
{
    public class SingletonDiagramVisitor : ANTLRv4ParserBaseVisitor<DiagramItem>
    {
        ...
        {
            var atomText = context.lexerAtom().GetText();
            var term = new Terminal(atomText, noneString, noneString);
            var ebnf = context.ebnfSuffix();
            if (ebnf == null)
                return term;
            ...
            switch (ebnf.GetText())
            {
                case "?": //TODO Add ?
                case "+":
                    return new OneOrMore(FSharpChoice<DiagramItem, string>.NewChoice1Of2(term), FSharpOption<FSharpChoice<DiagramItem, string>>.None);
                case "*":
                    return ZeroOrMore(FSharpChoice<DiagramItem, string>.NewChoice1Of2(term), FSharpOption<FSharpChoice<DiagramItem, string>>.None, true);
                default:
                    return term;
            }
        }
